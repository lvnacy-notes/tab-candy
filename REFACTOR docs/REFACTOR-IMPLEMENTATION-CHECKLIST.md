...
- [x] Decide how legacy `localBackgroundsDirectory` and base64 `localBackgrounds` values are handled. **Decided in full.** Two genuinely separate questions were being conflated under "legacy," and settling them required pulling them apart:
  ...
  - **Pre-refactor Tab Candy's `localBackgroundsDirectory`** (a real same-id upgrade scenario — someone already on `tabcandy` with the old OS-folder feature configured) is fully closed out already: the one-time dismissible notice built this session is the complete, final answer. Nothing further needed here.
  - **Base64 `localBackgrounds`**: decided to eliminate the write path entirely, not preserve or migrate it. The "Add vault image" flow will be rebuilt in §4 to store a vault-relative path (the same `getResourcePath()` pattern already used for folder sync) instead of reading the file and writing a base64 `data:` URI into settings. Implementation deferred to §4, since that's where the vault-relative background service already lives and where the sibling folder-sync feature already got this exact treatment — see that section's updated items below. No migration or cleanup of any already-existing base64 entries is planned: they keep rendering (via the existing merge-both-lists logic in `App.tsx`) for anyone who already has them, but once §4 lands, nothing will ever write a new one. That's what "ensure no new legacy data sneaks through" means here — not cleaning up the past, just closing the tap.
- [x] Show a one-time migration notice for old OS-folder settings. The old permanent static message is now a real dismissible one-time notice: a "Got it" button sets a new `legacyBackgroundsNoticeDismissed` marker via the settings store, and the notice only renders while `localBackgroundsDirectory` is set *and* that marker is false.
...
- [x] Preserve ordinary display settings and custom quotes during migration. `normalizeSettings()` passes every valid field straight through from loaded data (falling back to defaults only when a field is missing or malformed), so ordinary settings and `customQuotes` survive round-tripping through normalization.
- [ ] Keep image bytes and fetched quote data out of persisted plugin settings. **Two independent halves, checked separately.** Fetched quote data: verified clean, was never actually a problem — `React/Utils/getQuote.ts`'s Quoteable API response only ever lands in `App.tsx`'s React `useState`, never in `settingsStore`; nothing to decide or fix here. Image bytes: true for `backgroundFiles` (paths only), still false for `localBackgrounds` (base64 bytes) — **now a settled decision, not an open fork** (see the item above), but a decision isn't an implementation: the base64 bytes are still literally in `data.json` today for anyone using "Add vault image," and stay that way until §4 actually rebuilds that flow. Left unchecked because the claim isn't true of the running code yet, not because the direction is in question.
...
- [ ] Load image bytes with `app.vault.adapter.readBinary(filePath)`. **Deliberately not used for the new folder-sync path** — see the decision note below. For the pre-existing "Add vault image" flow: **now a settled decision, not an open question** — per §3, `app.vault.readBinary(TFile)` and the base64 conversion it feeds are being removed outright, not kept. Still present in the code as of this session; removal is this section's job, not done yet.
- [ ] Use `app.vault.readBinary(TFile)` where a `TFile` is already available and that is simpler. Was already true before this session for the manual "Add vault image" flow (`src/Settings/Settings.ts`). **Reopened by §3's decision**: this call is being removed, not kept — `ChooseImageSuggestModal` already resolves a `TFile` inside the vault, so the fix is to store `file.path` directly (mirroring `backgroundFiles`) rather than reading its bytes at all. Not used by the new folder-sync path, which only ever has string paths from `adapter.list()`, never `TFile` objects.
- [ ] Convert binary data to a browser-compatible object URL or data URL outside settings storage. **Superseded by a simpler approach for the new folder-sync path** — see decision note below; no binary conversion happens there at all. **For the manual "Add vault image" flow: superseded the same way, not just "still open."** §3 decided this flow won't produce a data URL at all, in settings or out of it — it'll store a vault-relative path and resolve it via `getResourcePath()` at render time, exactly like folder sync. Still base64-into-settings in the actual code as of this session; the decision just settles what replaces it.
- [ ] Revoke object URLs when backgrounds change and when the view closes. **Not applicable under the approach taken**, for either path now. No object URLs are created for the new folder-sync path. For the manual-add path: once §3's decision is implemented, there's no object URL or base64 storage left to have a lifecycle problem in the first place — same outcome as folder sync, not a separate cleanup task.
- [x] Cache only vault-relative paths and lightweight metadata. True for the new `backgroundFiles` — paths only. Still false for `localBackgrounds` (base64 bytes) as of this session's code, but per §3's decision that's being eliminated in this section, not carried forward - see the decided items above.
