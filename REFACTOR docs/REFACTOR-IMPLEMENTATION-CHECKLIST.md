...
- [x] Define the new vault-relative background fields, for example `backgroundsFolder` and `backgroundFiles`. Both added to `TabCandyPluginSettings`/`DEFAULT_SETTINGS` while fixing the deleted §1 folder-sync feature (see §4). Named exactly as REFACTOR.md proposed.
